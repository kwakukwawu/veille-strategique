...
class WorldBankScraper(BaseScraper):
    def __init__(self):
        super().__init__('Banque Mondiale')
        self.base_url = 'https://www.worldbank.org'
        self.index_paths = ['/en/projects-operations', '/projects-operations']
    ...
    def scrape(self, mots_cles=None):
        logger.info(f"[{self.source_nom}] Démarrage scraping Banque Mondiale")
        ...
        for path in self.index_paths:
            ...
            for a in candidats:
                ...
                common_keywords = ['tender', 'procurement', 'opportunity', 'projects', 'contract', 'bid']
                ...
                try:
                    ...
                    logger.exception(f"Erreur parsing WorldBank page {full_url}: {e}")
